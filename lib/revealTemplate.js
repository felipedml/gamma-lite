...
/**
 * Renderiza um slide de acordo com o tipo de template.
 * @param {string} template   Tipo: "clean", "sidebar", "imageFocus" ou "statement".
 * @param {string} title      Título do slide.
 * @param {string[]} bullets  Lista de bullets.
 * @param {string} imageUrl   URL da imagem (opcional).
 * @returns {Object}          Objeto { html: string }.
 */
export function renderSlideByTemplate(template, title, bullets = [], imageUrl) {
  ...
  // default ou "clean"
  return {
    html: `<h2>${escapeHtml(title)}</h2><ul>${bullets.map((b) => `<li>${escapeHtml(b)}</li>`).join("")}</ul>`
  };
}
